...
> 당장 구현 대상은 아니며, 아이디어 단계로 기록해둔다.

### 여행 큐레이션 + 리뷰 시스템

**컨셉**: 정산 플로우와 여행 정보를 자연스럽게 연결. 단순 정산 도구를 넘어 "여행 동반자 앱"으로 포지셔닝.

**핵심 기능**:
- **지역별 큐레이션**: 춘천 카페 리스트, 부산 현지 맛집 등 여행지별 장소 추천
- **정산 후 리뷰 유도**: 정산 완료 시 "방문한 장소에 리뷰를 남겨보세요" 프롬프트
- **실결제 기반 리뷰**: 실제 정산 내역이 있는 사용자의 리뷰 → 신뢰도 높은 후기 확보
- **장소 태깅**: 결제 항목에 장소 정보를 연결하여 자연스럽게 데이터 축적

**차별점**:
- 네이버/카카오맵 리뷰와 달리 "실제 돈을 쓴 사람"의 리뷰만 모임
- 정산 과정에서 자연스럽게 리뷰 데이터가 쌓이는 구조 (별도 리뷰 앱 불필요)
- 그룹 여행 컨텍스트의 추천 (혼자 vs 단체 여행 구분)

**수익화 연계**:
- 장소 큐레이션 내 제휴 광고 (Phase 2 수익 모델과 연결)
- 프리미엄 큐레이션 콘텐츠 (Phase 3 구독 모델과 연결)
