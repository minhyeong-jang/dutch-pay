...
export function TemplateContent() {
  ...
  const { data: template } = useSuspenseQuery(trpc.template.byId.queryOptions({ id }));

  const invalidate = () => {
    void queryClient.invalidateQueries({ queryKey: trpc.template.byId.queryKey({ id }) });
  };

  const addParticipantMutation = useMutation(
    trpc.participant.create.mutationOptions({ onSuccess: invalidate })
  );
  const removeParticipantMutation = useMutation(
    trpc.participant.delete.mutationOptions({ onSuccess: invalidate })
  );
  const addPaymentMutation = useMutation(
    trpc.payment.create.mutationOptions({ onSuccess: invalidate })
  );
  const removePaymentMutation = useMutation(
    trpc.payment.delete.mutationOptions({ onSuccess: invalidate })
  );
    ...
    <div className="flex flex-col gap-4 py-4">
      ...
      <div className="flex flex-col gap-4">
        ...
        <ParticipantList
          ...
          onRemove={(participantId) => removeParticipantMutation.mutate(participantId)}
        />
      </div>
          ...
          <TabsContent value="payments" className="mt-4">
            <div className="flex flex-col gap-4">
              <PaymentList
                ...
                participants={template.participants}
                onDelete={(paymentId) => removePaymentMutation.mutate(paymentId)}
              />
              <Button
                className="w-full"
